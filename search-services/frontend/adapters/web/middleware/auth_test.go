...
func TestValidateToken(t *testing.T) {
	testCases := []struct {
		...
		},
		{
			...
			prepareTokenAndAuth: func(t *testing.T) (string, *middleware.JwtAuthenticator) {
				auth, err := middleware.NewJwtAuthenticator(validUser, validPassword, jwtSecret, time.Millisecond)
				require.NoError(t, err)
				token, err := auth.CreateToken(validUser, validPassword)
				require.NoError(t, err)
		...
		},
		{
			...
			prepareTokenAndAuth: func(t *testing.T) (string, *middleware.JwtAuthenticator) {
				...
				otherAuth, err := middleware.NewJwtAuthenticator(validUser, validPassword, "invalid-secret", time.Hour)
				require.NoError(t, err)
...
func TestCheckToken(t *testing.T) {
	testCases := []struct {
		...
		{
			desc: "success - valid token in cookie",
		...
		{
			desc: "error - invalid token in cookie",
			prepareRequest: func(t *testing.T, auth *middleware.JwtAuthenticator) *http.Request {
				req := httptest.NewRequest(http.MethodGet, "/admin", nil)
				req.AddCookie(&http.Cookie{Name: "jwt_token", Value: "invalid.token.string"})
				return req
			},
			expectedStatus: http.StatusSeeOther,
			expectRedirect: true,
		...
		{
			desc: "error - expired token in cookie",
			prepareRequest: func(t *testing.T, auth *middleware.JwtAuthenticator) *http.Request {
				shortAuth, err := middleware.NewJwtAuthenticator(validUser, validPassword, jwtSecret, time.Millisecond)
				require.NoError(t, err)
		...
		{
			desc: "error - empty token in cookie",
			prepareRequest: func(t *testing.T, auth *middleware.JwtAuthenticator) *http.Request {
				req := httptest.NewRequest(http.MethodGet, "/admin", nil)
				req.AddCookie(&http.Cookie{Name: "jwt_token", Value: ""})
				return req
			},
			expectedStatus: http.StatusSeeOther,
			expectRedirect: true,
